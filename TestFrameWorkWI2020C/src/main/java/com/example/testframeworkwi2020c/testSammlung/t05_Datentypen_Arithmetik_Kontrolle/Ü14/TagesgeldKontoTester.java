...
public class TagesgeldKontoTester {
    List<Pair<String, Object>> objectList = new ArrayList<>();
    String jarFilePath;
    String className = "TagesgeldKonto";

    /***
     * Konstruktor
     * @param jarFilePath Festlegung der Variable jarFilePath
     */
    public TagesgeldKontoTester(String jarFilePath) { this.jarFilePath = jarFilePath; }
    ...
    public TestResult<String> testEinzahlen() throws Exception {
        ...
        // Verzinsung und festlegung des erwarteteten Wertes
        double expectedValue;
        if((double) priorValue < 1000){
            expectedValue = (double) priorValue + 500 + ((double)priorValue*0.1);
        }else if((double) priorValue < 3000){
            expectedValue = (double) priorValue + 500 + ((double)priorValue*0.2);
        }else if((double) priorValue < 10000){
            expectedValue = (double) priorValue + 500 + ((double)priorValue*0.3);
        }else{
            expectedValue = (double) priorValue + 500;
        }
