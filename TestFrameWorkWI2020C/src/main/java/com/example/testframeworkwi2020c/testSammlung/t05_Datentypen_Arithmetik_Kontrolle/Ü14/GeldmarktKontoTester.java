...
public class GeldmarktKontoTester {
    List<Pair<String, Object>> objectList = new ArrayList<>();
    String jarFilePath;
    String className = "GeldmarktKonto";

    /***
     * Konstruktor
     * @param jarFilePath Festlegung der Variable jarFilePath
     */
    public GeldmarktKontoTester(String jarFilePath) { this.jarFilePath = jarFilePath; }
    ...
    public TestResult<String> testEinzahlen() throws Exception {
        ...
        CodeRunnerBackend.setVariableValue(objectList,className,"kontostand", 0);

        // Test, ob random richtig benutzt wird
        for(int i=0; i<10; i++){
            CodeRunnerBackend.invokeMethodByName(objectList,className,"einzahlen", new Object[]{500}, new Class[]{int.class});
            Object shortTimeValue = CodeRunnerBackend.getVariableValue(objectList,className,"kontostand");
            if((double)shortTimeValue < (500*-10) || (double) shortTimeValue > (500*10)){
                return new TestResult<>(false, "ein falscher Faktor für die Berechnung des neuen Kontostandes benutzt und der Variablenwert daher um "+shortTimeValue.toString().trim());
            }
        }
